/*
* gargantua/physics/physics_physicssystem.cpp
*/
...
module gargantua.physics.physics_ecs_system;

import gargantua.log;

namespace gargantua::physics
{
	auto PhysicsECSSystem::Run(const time::TimeStep& ts, ecs::ECSSystem& ecs_s) -> void
	{
		const f32 dt = static_cast<f32>(ts.Seconds());
	...
	auto PhysicsECSSystem::DetectCollisions(ecs::ECSSystem& ecs_s) -> void
	{
	...
	auto PhysicsECSSystem::ResolveCollisions(ecs::ECSSystem& ecs_s) -> void
	{
