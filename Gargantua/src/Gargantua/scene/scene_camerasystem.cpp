/*
* gargantua/scene/scene_camerasystem.cpp
*/
...
{
	auto CameraSystem::Create(const std::string_view name, u32 viewport_width, u32 viewport_height) -> ecs::entity_t
	{
	...
	auto CameraSystem::Destroy(const std::string_view name) -> void
	{
	...
	auto CameraSystem::Get(const std::string_view name) -> ecs::entity_t
	{
		auto g = ecs_system->Group<TagComponent, render::CameraComponent>();
		auto v = g.View();
		for (auto e : v)
