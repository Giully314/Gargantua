/*
* gargantua/render/render_quadbatch.cpp
* 
...
module gargantua.render.quad_batch;
...
{
	QuadBatch::QuadBatch(const u32 max_num_of_quads) : max_quads_per_batch(max_num_of_quads),
		...
		current_num_of_textures(0)
	{
		...
		// Load white texture
		shared_res<Texture> texture = TextureSystem::Instance().Get("WhiteTexture");
	...
	auto QuadBatch::Add(const math::Mat4df& transform, const SubTexture& subtexture,
		f32 tiling_factor) -> void
	{
		// Check if the texture is already present.
		u32 texture_idx = 0;
		const auto& texture = subtexture.MainTexture();
		...
		const math::Vec4df white{ 1.0f, 1.0f, 1.0f, 1.0f };
		const auto texture_coords = subtexture.Coords();
		for (u32 i = 0; i < data.vertices.size(); ++i)
		{
			auto v = transform * data.vertices[i];
			positions.push_back(std::move(v));
